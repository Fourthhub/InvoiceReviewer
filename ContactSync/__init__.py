...
def obtener_acceso_hostaway():
    logging.info("Obteniendo token de acceso de Hostaway...")
    try:
        payload = {
...
def retrieveReservations(arrivalStartDate, arrivalEndDate, token):
    logging.info(f"Recuperando reservas entre {arrivalStartDate} y {arrivalEndDate}")
    ...
    response.raise_for_status()
    data = response.json()
    num_reservas = len(data.get("result", []))
    logging.info(f"{num_reservas} reservas recuperadas.")
    return data

def obtener_fechas():
    fecha_actual = datetime.now()
    fecha_hace_dos_semanas = fecha_actual - timedelta(weeks=3)
    logging.info(f"Fechas calculadas: actual={fecha_actual.strftime('%Y-%m-%d')}, hace 2 semanas={fecha_hace_dos_semanas.strftime('%Y-%m-%d')}")
    return fecha_actual.strftime('%Y-%m-%d'), fecha_hace_dos_semanas.strftime('%Y-%m-%d')

def comprobar_si_existe_factura(reserva):
    logging.info(f"Comprobando si existe factura para la reserva {reserva.get('hostawayReservationId')}")
    custom_fields = reserva["customFieldValues"]
    for field in custom_fields:
        if field["customField"]["name"] == "holdedID" and field["value"] == "Ya esta facturada":
            logging.info("La reserva ya está facturada.")
            return True
    logging.info("La reserva no está facturada aún.")
    return False

def determinar_serie_y_iva(reserva, token):
    logging.info(f"Determinando serie e IVA para la reserva {reserva.get('hostawayReservationId')}")
    ...
    logging.info(f"Método de pago encontrado: {payment_method}")
    if payment_method == "cash":
        logging.info("Pago en efectivo: serie 'Efectivo', IVA 0%")
        return "Efectivo", 0
    ...
    logging.info(f"Serie: {serie_facturacion}, IVA: {iva}")
    return serie_facturacion, iva

def marcarComoFacturada(reserva, token):
    logging.info(f"Marcando reserva {reserva.get('hostawayReservationId')} como facturada.")
    encontrado = False
    try:
        reserva_id = str(reserva["hostawayReservationId"])
        ...
        if not encontrado:
            logging.info("Campo 'holdedID' no encontrado. Añadiendo nuevo campo.")
            nuevoCustomField = {"customFieldValues": [{"customFieldId": 56844, "value": "Ya esta facturada"}]}
            response = requests.put(url, json=nuevoCustomField, headers=headers)
        else:
            logging.info("Campo 'holdedID' actualizado. Enviando reserva actualizada.")
            response = requests.put(url, json=reserva, headers=headers)

        response.raise_for_status()
        logging.info("Reserva marcada como facturada correctamente.")
...
def crear_factura(reserva, serie_facturacion, iva):
    logging.info(f"Creando factura para la reserva {reserva.get('hostawayReservationId')}")
    try:
        now = datetime.now()
        ...
        response.raise_for_status()
        logging.info(f"Factura creada correctamente. Código de estado: {response.status_code}")
...
def main(mytimer: func.TimerRequest) -> None:
    logging.info("Inicio de ejecución del Timer Trigger.")

    try:
        access_token = obtener_acceso_hostaway()
        principio, final = obtener_fechas()

        reservas_response = retrieveReservations(arrivalStartDate=final, arrivalEndDate=principio, token=access_token)
        listaReservas = reservas_response.get("result", [])
        logging.info(f"Total de reservas a procesar: {len(listaReservas)}")

        for reserva in listaReservas:
            logging.info(f"Procesando reserva {reserva.get('hostawayReservationId')}, estado de pago: {reserva.get('paymentStatus')}")
            if reserva.get("paymentStatus") != "Paid":
                logging.info("Reserva no pagada. Se omite.")
                continue
            ...
            logging.info(f"Resultado creación factura: {resultado_crear_factura}, ID factura: {factura_info.get('id', 'desconocido')}")

            resultado_marcar = marcarComoFacturada(reserva, access_token)
            logging.info(resultado_marcar)

        logging.info("Finalización correcta del procesamiento de reservas.")
